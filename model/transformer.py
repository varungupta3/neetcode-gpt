...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.vnn = self.VanillaNeuralNetwork(model_dim=model_dim)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        attention_output = self.mhsa(self.ln1(embedded)) # (B, T, model_dim)
        residual_connection_output = torch.add(embedded, attention_output) # (B, T, model_dim)
        feed_forward_output = self.vnn(self.ln2(residual_connection_output)) # (B, T, model_dim)
        return torch.round(torch.add(residual_connection_output, feed_forward_output), decimals=4) # (B, T, model_dim)
